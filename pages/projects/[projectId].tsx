...
type Status = 'contacted' | 'uncontacted' | 'toBeContacted';
...
const Project = ({ projectId }: ProfileProps) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    enabled: !!projectId,
    onSuccess: (data) => setProjectTitle(data.project.name),
  });
                    ...
                    <div className="h-10 w-10">
                      <Avatar.Root className="AvatarRoot">
                        ...
                        <Avatar.Fallback
                          ...
                          delayMs={600}
                        >
                          <div className="bg-gradient-3 animate-pulse h-full w-full rounded-full" />
                        </Avatar.Fallback>
                      </Avatar.Root>
                    </div>
                    <div>
...
const StatusTags: React.FC<{ status: Status }> = ({ status }) => {
  return (
    <div
      className={clsx(
...
const ProjectHeader = ({ value, onChange }: CustomInputProps) => {
  const isMobile = useMobile();
  const [focusInput, setFocusInput] = React.useState(false);
  const inputRef = React.useRef<HTMLInputElement>(null!);

  useClickOutside(inputRef, () => setFocusInput(false));
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!value) onChange('Untitled Project');
    setFocusInput(false);
  };
