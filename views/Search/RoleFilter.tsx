...
export const RoleFilter = ({ handleGoBack, handleSelect }: RoleFilterProps) => {
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      className="flex h-[434px] w-full !cursor-default flex-col rounded-3xl bg-white p-6 shadow-base md:h-[434px] md:w-[500px] lg:h-[390px]"
    >
      ...
      <div>
        <ul className="grid grid-cols-3 gap-3">
          {roles.map((role) => (
            <li
              key={role.title}
              className="flex h-[100px] max-h-[100px] cursor-pointer flex-col items-center justify-center space-y-2 rounded-lg border"
              onClick={() =>
                handleSelect('role_' + role.title.split(' ').join('_'))
              }
            >
